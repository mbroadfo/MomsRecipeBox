...
// Lazy loading function for AUTH0_DOMAIN to ensure it's available after secrets initialization
function getAuth0Domain() {
  const AUTH0_DOMAIN = process.env.AUTH0_DOMAIN;
  
  // Check if domain is properly configured
  if (!AUTH0_DOMAIN || AUTH0_DOMAIN === '$' || AUTH0_DOMAIN.includes('${')) {
    console.error('❌ AUTH0_DOMAIN not properly configured:', AUTH0_DOMAIN);
    throw new Error('AUTH0_DOMAIN environment variable is not properly configured. Check AWS Secrets Manager.');
  }
...
// Cache for JWKS client (created dynamically when needed)
let jwksClientCache = null;

// Create JWKS client dynamically when needed
function getJwksClient() {
  if (!jwksClientCache) {
    const AUTH0_DOMAIN = getAuth0Domain();
    const JWKS_URL = `https://${AUTH0_DOMAIN}/.well-known/jwks.json`;
    
    console.log('🔧 JWT Validator AUTH0_DOMAIN:', AUTH0_DOMAIN);
    console.log('🔧 JWT Validator JWKS_URL:', JWKS_URL);
...
export function validateJWT(token, requiredAudience = null) {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      console.error('❌ AUTH0_DOMAIN configuration error:', domainError.message);
      reject(domainError);
    }
  });
