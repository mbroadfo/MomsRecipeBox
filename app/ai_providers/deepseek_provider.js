...
export class DeepSeekProvider extends BaseAIProvider {
  ...
  /**
   * Format messages for DeepSeek's API (OpenAI compatible)
   */
  ...
  /**
   * Format payload for DeepSeek's API
   */
  ...
  async makeRequest(systemMessage, userMessages, temperature = 0.7) {
    ...
    this.logApiOperation('REQUEST', `Making API request to DeepSeek with model ${config.model}`);
  ...
  /**
   * Handle URL extraction - same implementation as OpenAI provider
   */
...
${textContent.substring(0, 8000)} // Limit to 8k characters to stay within token limits`;
  ...
  /**
   * Handle chat message - same implementation as OpenAI provider
   */
  ...
  /**
   * Handle pasted recipe content - same implementation as OpenAI provider
   */
